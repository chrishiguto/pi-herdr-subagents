// Ported near-verbatim from Herdr's generated `herdr-agent-state.ts` Pi integration.
// Upstream markers: HERDR_INTEGRATION_ID=pi, HERDR_INTEGRATION_VERSION=8.
//
// This is deliberately not a Pi extension entry point. `extensions/herdr-subagents/index.ts` composes it into
// pi-herdr-subagents so the package appears as one loaded extension. Keep the
// filename and protocol behavior aligned with the upstream integration when
// updating the port.
//
// Adaptation from upstream: mutable reporter state is scoped to registration so
// extension-harness tests and replacement runtimes cannot share session state.
...
/** Compose Herdr's Pi state integration into the pi-herdr-subagents runtime. */
