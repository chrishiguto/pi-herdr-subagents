...
describe("HerdrEventStream", () => {
  ...
  it("reconciles after the initial subscription so pre-connect gaps are covered", async () => {
    server = await FakeHerdrServer.start();
    controller = new AbortController();
    const subscribed = server.nextSubscribed();
    stream = createHerdrEventStream({
    ...
    await subscribed;
    await waitFor(() => stream!.connected && reconciles === 1);
  });
  ...
  it("dispatches pane moves to the previous pane watcher", async () => {
    const { stream, conn } = await connect();
    await waitFor(() => stream.connected);
    ...
    conn.push({
      data: {
        ...
        pane: { pane_id: "w2:p7", terminal_id: "term_123" },
      },
      event: "pane_moved",
    });
  ...
  it("dispatches only released pane_agent_detected events", async () => {
    const { stream, conn } = await connect();
    await waitFor(() => stream.connected);
    ...
    await waitFor(() => received.length > 0);
    assert.deepEqual(received, [{ event: "pane_agent_released", paneId: "w1:p4" }]);
  });
