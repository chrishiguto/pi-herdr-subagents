 ...
 * pi-interactive-subagents) are never loaded. Model-backed tests copy auth and
 * their test agent definitions; no-model tests create neither.
 *
 * The extension under test is force-loaded from the working tree via
 * `pi -ne -e <abs extensions/herdr-subagents/index.ts>` (same rationale as the reference harness,
...
export function integrationPrereqs(
  opts: { noModel?: boolean } = {},
): { ok: boolean; reason?: string } {
  if (process.env.PI_RUN_HERDR_INTEGRATION !== "1") {
    return {
      ok: false,
  ...
  if (!opts.noModel && !existsSync(join(REAL_AGENT_DIR, "auth.json"))) {
...
/** Herdr >=0.8.2 waits for the fresh pane shell and Pi readiness server-side. */
export const startPiAgent = (
  ts: TestSession,
  params: Parameters<HerdrClient["agentStart"]>[0],
) => ts.client.agentStart(params);
...
export interface TestSession {
  ...
  tmpDir: string;
  /** The isolated Herdr UI pane from which background panes are split. */
  sourcePaneId: string;
...
export async function createTestSession(
  ...
  const prereqs = integrationPrereqs(opts);
  ...
  // Temp dirs: scratch + isolated Pi config. Provider-free probes intentionally
  // do not read/copy auth and do not create an agents directory.
  ...
  tmux(["set-option", "-t", `=${tmuxSession}`, "extended-keys", "on"]);
  ...
  const teardownSync = () => {
    ...
    const managementEnv = { ...herdrEnv };
    delete managementEnv.HERDR_SESSION;
 ...
 * working-tree extension (`-ne -e`) and the temp config dir. Herdr 0.8 first
 * splits the isolated source pane, then starts the recognized Pi agent in that
 * existing shell pane. The initial prompt is passed as a positional message.
 */
export async function startOrchestrator(
  ...
  opts: {
    ...
    name?: string;
    cwd?: string;
    /** Extra environment for the orchestrator pane. */
  ...
  const started = await startPiAgent(ts, {
    liveAgentName: makeLiveAgentName(name, id.slice(0, 16)),
    paneId: pane.pane_id,
    argv,
  });
  await ts.client.agentPrompt(started.name, opts.prompt);
  ts.trackedPanes.push(started.paneId);
  return { paneId: started.paneId, sessionFile };
}
...
/** True while Herdr still recognizes the named live agent. */
export async function agentExists(ts: TestSession, liveAgentName: string): Promise<boolean> {
  const exec = herdrExecEnv(ts.herdrEnv);
  const result = await exec(HERDR_BIN, ["agent", "get", liveAgentName]);
  if (result.code === 0) return true;
  const output = `${result.stdout}\n${result.stderr}`;
  if (/agent_not_found|not found/i.test(output)) return false;
  throw new Error(`herdr agent get failed: ${output.trim()}`);
}
...
/** Raw pane-level key input (`herdr pane send-keys`) for user-simulation steps. */
export async function paneSendKeys(ts: TestSession, paneId: string, keys: string[]): Promise<void> {
  const exec = herdrExecEnv(ts.herdrEnv);
  const result = await exec(HERDR_BIN, ["pane", "send-keys", paneId, ...keys]);
  if (result.code !== 0) throw new Error(`pane send-keys failed: ${result.stderr}`);
}
